...
export default function ParentSessionDetailPage({
  ...
  useEffect(() => {
    fetch(`/api/parent/children/${id}/feedback?sessionId=${sessionId}`)
      .then((r) => r.json())
  ...
  if (!fb) return (
    <div className="space-y-4">
      <Link href={`/parent/players/${id}`} className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "gap-2")}><ArrowLeft className="h-4 w-4" />Back</Link>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        <Link href={`/parent/players/${id}`} className={cn(buttonVariants({ variant: "ghost", size: "icon" }))}><ArrowLeft className="h-4 w-4" /></Link>
