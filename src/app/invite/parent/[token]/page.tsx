...
export default function ParentInvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setError(null);
    const res = await fetch(`/api/invite/parent/${token}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: name || `${playerName}'s Parent`, email, password }),
    });
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl">Parent Invite</CardTitle>
          <CardDescription>{coachName} has invited you as a parent on CoachCRM</CardDescription>
        </CardHeader>
        <CardContent>
          {error === "Invite already accepted" ? (
            <div className="text-center space-y-3">
              <p className="text-muted-foreground">This invite has already been accepted.</p>
              <Button onClick={() => router.push("/parent/login")}>Go to Parent Login</Button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="rounded-lg bg-muted p-3 text-center">
                <p className="text-sm text-muted-foreground">You&apos;ll be linked to:</p>
              ...
              <Button type="submit" className="w-full gap-2" disabled={submitting}>
                {submitting && <Loader2 className="h-4 w-4 animate-spin" />}Create Parent Account
              </Button>
            </form>
          )}
        </CardContent>
